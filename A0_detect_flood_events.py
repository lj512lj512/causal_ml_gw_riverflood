...
def load_dkm_dynamics(DKM_id, jumpfilled = False) -> xr.Dataset:
    ...
    if jumpfilled:
        ...
        dtp = xr.load_dataset(os.path.join(dataPath, f'DK{DKM_id}_HIP_{resolution}m_2DSZ_dtp_jumps_filled_1989_2024.nc'))
    else:
...
def load_camels_info():
    ...
    attributes = pd.read_csv(
        os.path.join(CAMELS_FOLDER, "Attibutes", "CAMELS_DK_topography.csv")
    )
